...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable {
	...
	private int networkTickCount, field_175090_f, floatingTickCount;
	...
	public NetHandlerPlayServer(MinecraftServer server, NetworkManager networkManager, EntityPlayerMP player) {
	...
	public void sendPacket(final Packet packet) {
		if (packet instanceof S02PacketChat) {
			...
			EntityPlayer.EnumChatVisibility visibility = this.playerEntity.getChatVisibility();

			if (visibility == EntityPlayer.EnumChatVisibility.HIDDEN) {
				return;
			}

			if (visibility == EntityPlayer.EnumChatVisibility.SYSTEM && !s02packetchat.isChat()) {
				return;
			}
		}
		...
		try {
			...
			crashreportcategory.addCrashSectionCallable("Packet class", () -> packet.getClass().getCanonicalName());
		...
		switch (packet.getAction()) {
			...
			case RIDING_JUMP:
				if (this.playerEntity.ridingEntity instanceof EntityHorse) {
					((EntityHorse) this.playerEntity.ridingEntity).setJumpPower(packet.getAuxData());
				}
				break;
			case OPEN_INVENTORY:
				if (this.playerEntity.ridingEntity instanceof EntityHorse) {
	...
	public void processClientStatus(C16PacketClientStatus packet) {
		PacketThreadUtil.checkThreadAndEnqueue(packet, this, this.playerEntity.getServerForPlayer());
		this.playerEntity.markPlayerActive();
		C16PacketClientStatus.EnumState enumState = packet.getStatus();
	...
	public void processVanilla250Packet(C17PacketCustomPayload packet) {
		...
		if ("MC|BEdit".equals(packet.getChannelName())) {
			PacketBuffer packetBuffer = new PacketBuffer(Unpooled.wrappedBuffer(packet.getBufferData()));
			...
			try {
				...
				if (itemStack != null) {
					...
					if (itemStack.getItem() == Items.written_book && currentItem.getItem() == Items.writable_book) {
						currentItem.setTagInfo("author", new NBTTagString(this.playerEntity.getCommandSenderName()));
			...
			if (!this.serverController.isCommandBlockEnabled()) {
				...
				try {
					...
					CommandBlockLogic blockLogic = null;
					...
					if (j == 0) {
						...
						if (tileentity instanceof TileEntityCommandBlock) {
							blockLogic = ((TileEntityCommandBlock) tileentity).getCommandBlockLogic();
						}
					} else if (j == 1) {
						Entity entity = this.playerEntity.worldObj.getEntityByID(packetBuffer.readInt());

						if (entity instanceof EntityMinecartCommandBlock) {
							blockLogic = ((EntityMinecartCommandBlock) entity).getCommandBlockLogic();
						}
					}
					...
					if (blockLogic != null) {
						blockLogic.setCommand(s1);
						blockLogic.setTrackOutput(flag);

						if (!flag) {
							blockLogic.setLastOutput(null);
						}

						blockLogic.updateCommand();
		...
		} else if ("MC|ItemName".equals(packet.getChannelName()) && this.playerEntity.openContainer instanceof ContainerRepair) {
			...
			if (packet.getBufferData() != null && packet.getBufferData().readableBytes() >= 1) {
				String s = ChatAllowedCharacters.filterAllowedCharacters(packet.getBufferData().readStringFromBuffer(32767));
