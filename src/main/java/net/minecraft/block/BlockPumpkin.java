...
public class BlockPumpkin extends BlockDirectional
{
    ...
    private void trySpawnGolem(World world, BlockPos pos)
    {
        ...
        if ((blockpattern$patternhelper = this.getSnowmanPattern().match(world, pos)) != null)
        {
        ...
        else if ((blockpattern$patternhelper = this.getGolemPattern().match(world, pos)) != null)
        {
            for (int k = 0; k < this.getGolemPattern().getPalmLength(); ++k)
            {
                for (int l = 0; l < this.getGolemPattern().getThumbLength(); ++l)
                {
                    world.setBlockState(blockpattern$patternhelper.translateOffset(k, l, 0).getPos(), Blocks.air.getDefaultState(), 2);
                }
            }
