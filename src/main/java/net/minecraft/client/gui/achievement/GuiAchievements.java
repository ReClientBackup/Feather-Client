...
public class GuiAchievements extends Screen implements IProgressMeter
{
    ...
    {
        if (keyCode == getMc().gameSettings.keyBindInventory.getKeyCode())
        {
    ...
    protected void drawAchievementScreen(int p_146552_1_, int p_146552_2_, float p_146552_3_)
    {
        ...
        for (int l3 = 0; (float)l3 * f - (float)j2 < 155.0F; ++l3)
        {
            ...
            {
                random.setSeed(getMc().getSession().getPlayerID().hashCode() + k1 + i4 + (l1 + l3) * 16);
        ...
        if (achievement != null)
        {
            ...
            {
                int j8 = Math.max(getFr().getStringWidth(s), 120);
                int i9 = getFr().splitStringWidth(s1, j8);
                ...
                getFr().drawSplitString(s1, i7, k7 + 12, j8, -6250336);
            ...
            if (s != null)
            {
                getFr().drawStringWithShadow(s, (float)i7, (float)k7, this.statFileWriter.canUnlockAchievement(achievement) ? (achievement.getSpecial() ? -128 : -1) : (achievement.getSpecial() ? -8355776 : -8355712));
            }
        }
