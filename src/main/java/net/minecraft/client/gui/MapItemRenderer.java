...
public class MapItemRenderer
{
    ...
    class Instance
    {
        ...
        private void render(boolean noOverlayRendering)
        {
            ...
            worldrenderer.pos((float)(i + 0) + f, (float)(j + 128) - f, -0.009999999776482582D).tex(0.0D, 1.0D).func_181675_d();
            worldrenderer.pos((float)(i + 128) - f, (float)(j + 128) - f, -0.009999999776482582D).tex(1.0D, 1.0D).func_181675_d();
            worldrenderer.pos((float)(i + 128) - f, (float)(j + 0) + f, -0.009999999776482582D).tex(1.0D, 0.0D).func_181675_d();
            worldrenderer.pos((float)(i + 0) + f, (float)(j + 0) + f, -0.009999999776482582D).tex(0.0D, 0.0D).func_181675_d();
            tessellator.draw();
            ...
            for (Vec4b vec4b : this.mapData.playersVisibleOnMap.values())
            {
                if (!noOverlayRendering || vec4b.func_176110_a() == 1)
                {
                    ...
                    worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
                    float f5 = -0.001F;
                    worldrenderer.pos(-1.0D, 1.0D, (float)k * -0.001F).tex(f1, f2).func_181675_d();
                    worldrenderer.pos(1.0D, 1.0D, (float)k * -0.001F).tex(f3, f2).func_181675_d();
                    worldrenderer.pos(1.0D, -1.0D, (float)k * -0.001F).tex(f3, f4).func_181675_d();
                    worldrenderer.pos(-1.0D, -1.0D, (float)k * -0.001F).tex(f1, f4).func_181675_d();
                    tessellator.draw();
