...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handleChat(S02PacketChat packetIn)
    {
        ...
        {
            this.gameController.inGameScreen.setRecordPlaying(packetIn.getChatComponent().getUnformattedText(), false);
        }
        else
        {
            this.gameController.inGameScreen.getChatGUI().printChatMessage(packetIn.getChatComponent());
        }
    }
    ...
    public void handleEntityAttach(S1BPacketEntityAttach packetIn)
    {
        ...
        if (packetIn.getLeash() == 0)
        {
            ...
            if (flag)
            {
                ...
                this.gameController.inGameScreen.setRecordPlaying(I18n.format("mount.onboard", new Object[] {GameSettings.getKeyDisplayString(gamesettings.keyBindSneak.getKeyCode())}), false);
            }
        }
    ...
    public void handleTitle(S45PacketTitle packetIn)
    {
        ...
        this.gameController.inGameScreen.displayTitle(s, s1, packetIn.getFadeInTime(), packetIn.getDisplayTime(), packetIn.getFadeOutTime());
    }
    ...
    {
        this.gameController.inGameScreen.getTabList().setHeader(packetIn.getHeader().getFormattedText().length() == 0 ? null : packetIn.getHeader());
        this.gameController.inGameScreen.getTabList().setFooter(packetIn.getFooter().getFormattedText().length() == 0 ? null : packetIn.getFooter());
    }
        ...
        else
        {
            ...
            else
            {
                this.gameController.addScheduledTask(new Runnable()
                {
                    public void run()
                    {
                        NetHandlerPlayClient.this.gameController.displayGuiScreen(new YesNoScreen((result, id) -> {
