...
public class EntityPlayerMP extends EntityPlayer implements ICrafting {
    ...
    private int lastExperience = -99999999, respawnInvulnerabilityTicks = 60;
    ...
    public EntityPlayerMP(MinecraftServer server, WorldServer world, GameProfile profile, ItemInWorldManager interactionManager) {
        super(world, profile);
        ...
        try {
            ...
            if (this.getHealth() + this.getAbsorptionAmount() != this.combinedHealth) {
                ...
                for (ScoreObjective scoreobjective : this.getWorldScoreboard().getObjectivesFromCriteria(IScoreObjectiveCriteria.health)) {
                    this.getWorldScoreboard().getValueFromObjective(this.getCommandSenderName(), scoreobjective).func_96651_a(Collections.singletonList(this));
                }
            }
    ...
    protected void updateBiomesExplored() {
        ...
        JsonSerializableSet serializableSet = this.getStatFile().func_150870_b(AchievementList.exploreAllBiomes);

        if (serializableSet == null) {
            serializableSet = this.getStatFile().func_150872_a(AchievementList.exploreAllBiomes, new JsonSerializableSet());
        }

        serializableSet.add(s);

        if (this.getStatFile().canUnlockAchievement(AchievementList.exploreAllBiomes) && serializableSet.size() >= BiomeGenBase.explorationBiomesList.size()) {
            Set<BiomeGenBase> set = Sets.newHashSet(BiomeGenBase.explorationBiomesList);

            for (String s1 : serializableSet) {
    ...
    public void onDeath(DamageSource cause) {
        ...
        if (entitylivingbase != null) {
            ...
            if (eggInfo != null) {
                this.triggerAchievement(eggInfo.field_151513_e);
            }
    ...
    public void onItemPickup(Entity entity, int p_71001_2_) {
        super.onItemPickup(entity, p_71001_2_);
        this.openContainer.detectAndSendChanges();
    }
