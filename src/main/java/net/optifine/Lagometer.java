...
public class Lagometer {
   ...
   public static void showLagometer(ScaledResolution scaledResolution) {
      if(gameSettings != null) {
         if(gameSettings.ofLagometer || gameSettings.field_181657_aC) {
            ...
            InGameScreen.drawRect(l1 - 1, i2 - 1, l1 + 50, i2 + 10, -1605349296);
   ...
   private static long renderTime(int frameNum, long time, int r, int g, int b, float baseHeight, WorldRenderer worldRenderer) {
      long i = time / 200000L;
      if(i < 3L) {
         return 0L;
      } else {
         worldRenderer.pos((double)((float)frameNum + 0.5F), (double)(baseHeight - (float)i + 0.5F), 0.0D).color(r, g, b, 255).endVertex();
         worldRenderer.pos((double)((float)frameNum + 0.5F), (double)(baseHeight + 0.5F), 0.0D).color(r, g, b, 255).endVertex();
         return i;
      }
   }

   private static long renderTimeDivider(int frameStart, int frameEnd, long time, int r, int g, int b, float baseHeight, WorldRenderer worldRenderer) {
      long i = time / 200000L;
      if(i < 3L) {
         return 0L;
      } else {
         worldRenderer.pos((double)((float)frameStart + 0.5F), (double)(baseHeight - (float)i + 0.5F), 0.0D).color(r, g, b, 255).endVertex();
         worldRenderer.pos((double)((float)frameEnd + 0.5F), (double)(baseHeight - (float)i + 0.5F), 0.0D).color(r, g, b, 255).endVertex();
         return i;
      }
   }
