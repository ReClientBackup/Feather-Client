...
public class EnchantmentHelper {
    ...
    public static void applyThornEnchantments(EntityLivingBase livingBase, Entity entity) {
        ENCHANTMENT_ITERATOR_HURT.attacker = entity;
        ENCHANTMENT_ITERATOR_HURT.user = livingBase;

        if (livingBase != null) {
            applyEnchantmentModifierArray(ENCHANTMENT_ITERATOR_HURT, livingBase.getInventory());
        }

        if (entity instanceof EntityPlayer) {
            assert livingBase != null;
            applyEnchantmentModifier(ENCHANTMENT_ITERATOR_HURT, livingBase.getHeldItem());
        }
    }
                    ...
                    for (int l = k; randomIn.nextInt(50) <= l; l >>= 1) {
                        ...
                        while (iterator.hasNext()) {
                            ...
                            for (EnchantmentData data : list) {
                                if (!data.enchantmentobj.canApplyTogether(Enchantment.getEnchantmentById(integer))) {
                                    flag = false;
                                    break;
                                }
                            }
                        ...
                        if (!map.isEmpty()) {
                            EnchantmentData data = WeightedRandom.getRandomItem(randomIn, map.values());
                            list.add(data);
                        }
                    }
                }
