...
import com.murengezi.minecraft.block.state.BlockPistonStructureHelper;
...
public class BlockPistonBase extends Block {
    ...
    public IBlockState onBlockPlaced(World world, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
        return this.getDefaultState().withProperty(FACING, getFacingFromEntity(pos, placer)).withProperty(EXTENDED, false);
    }
    ...
    public static boolean canPush(Block block, World world, BlockPos pos, EnumFacing direction, boolean allowDestroy) {
        if (block == Blocks.obsidian) {
            ...
            if (pos.getY() <= world.getHeight() - 1 && (direction != EnumFacing.UP || pos.getY() != world.getHeight() - 1)) {
                if (block != Blocks.piston && block != Blocks.sticky_piston) {
                    if (block.getBlockHardness() == -1.0F) {
                        return false;
                    }
                ...
                } else if (world.getBlockState(pos).getValue(EXTENDED)) {
                    return false;
                }
    ...
    private boolean doMove(World world, BlockPos pos, EnumFacing direction, boolean extending) {
        ...
        if (!blockpistonstructurehelper.canMove()) {
            ...
            if (extending) {
                BlockPistonExtension.EnumPistonType pistonType = this.isSticky ? BlockPistonExtension.EnumPistonType.STICKY : BlockPistonExtension.EnumPistonType.DEFAULT;
                IBlockState iblockstate1 = Blocks.piston_head.getDefaultState().withProperty(BlockPistonExtension.FACING, direction).withProperty(BlockPistonExtension.TYPE, pistonType);
