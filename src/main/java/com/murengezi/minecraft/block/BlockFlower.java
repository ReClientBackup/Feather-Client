...
public abstract class BlockFlower extends BlockBush {
    ...
    public void getSubBlocks(Item item, CreativeTabs tab, List<ItemStack> list) {
        for (BlockFlower.EnumFlowerType flowerType : BlockFlower.EnumFlowerType.getTypes(this.getBlockType())) {
            list.add(new ItemStack(item, 1, flowerType.getMeta()));
        }
    }
    ...
    public enum EnumFlowerType implements IStringSerializable {
        ...
        public static BlockFlower.EnumFlowerType getType(BlockFlower.EnumFlowerColor blockType, int meta) {
            BlockFlower.EnumFlowerType[] flowerTypes = TYPES_FOR_BLOCK[blockType.ordinal()];

            if (meta < 0 || meta >= flowerTypes.length) {
                meta = 0;
            }

            return flowerTypes[meta];
        }
        ...
        static {
            for (final BlockFlower.EnumFlowerColor flowerColor : BlockFlower.EnumFlowerColor.values()) {
                Collection<BlockFlower.EnumFlowerType> collection = Collections2.filter(Lists.newArrayList(values()), p_apply_1_ -> p_apply_1_.getBlockType() == flowerColor);
                TYPES_FOR_BLOCK[flowerColor.ordinal()] = collection.toArray(new EnumFlowerType[0]);
            }
        }
    }
}
