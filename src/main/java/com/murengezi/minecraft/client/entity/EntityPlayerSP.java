...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    private boolean serverSneakState, serverSprintState;
    private int positionUpdateTicks;
    private boolean hasValidHealth;
    private String clientBrand;
    public MovementInput movementInput;
    protected Minecraft mc;
    protected int sprintToggleTimer, sprintingTicksLeft;
    public float renderArmYaw, renderArmPitch, prevRenderArmYaw, prevRenderArmPitch;
    private int horseJumpPowerCounter;
    private float horseJumpPower;
    public float timeInPortal, prevTimeInPortal;

    public EntityPlayerSP(Minecraft mc, World world, NetHandlerPlayClient netHandler, StatFileWriter statFile) {
        super(world, netHandler.getGameProfile());
    ...
    public void onUpdateWalkingPlayer() {
        ...
        if (this.isCurrentViewEntity()) {
            double deltaX = this.posX - this.lastReportedPosX;
            double deltaY = this.getEntityBoundingBox().minY - this.lastReportedPosY;
            double deltaZ = this.posZ - this.lastReportedPosZ;
            double deltaYaw = this.rotationYaw - this.lastReportedYaw;
            double deltaPitch = this.rotationPitch - this.lastReportedPitch;
            boolean updatePosition = deltaX * deltaX + deltaY * deltaY + deltaZ * deltaZ > 9.0E-4D || this.positionUpdateTicks >= 20;
            boolean updateRotation = deltaYaw != 0.0D || deltaPitch != 0.0D;
     ...
     * TODO 1.7 Animations
     */
    public void swingItemOnlyClient() {
        super.swingItem();
    }
    ...
    public void addChatComponentMessage(IChatComponent chatComponent) {
        this.mc.inGameScreen.getChatGUI().printChatMessage(chatComponent);
    }

    protected boolean pushOutOfBlocks(double x, double y, double z) {
        if (!this.noClip) {
            BlockPos blockpos = new BlockPos(x, y, z);
            double deltaX = x - (double) blockpos.getX();
            double deltaZ = z - (double) blockpos.getZ();

            if (!this.isOpenBlockSpace(blockpos)) {
                int i = -1;
                double d2 = 9999.0D;

                if (this.isOpenBlockSpace(blockpos.west()) && deltaX < d2) {
                    d2 = deltaX;
                    i = 0;
                }

                if (this.isOpenBlockSpace(blockpos.east()) && 1.0D - deltaX < d2) {
                    d2 = 1.0D - deltaX;
                    i = 1;
                }

                if (this.isOpenBlockSpace(blockpos.north()) && deltaZ < d2) {
                    d2 = deltaZ;
                    i = 4;
                }

                if (this.isOpenBlockSpace(blockpos.south()) && 1.0D - deltaZ < d2) {
                    i = 5;
                }
    ...
    public void setSprinting(boolean sprint) {
        ...
        this.sprintingTicksLeft = sprint ? 600 : 0;
    }
    ...
    public void addChatMessage(IChatComponent component) {
        this.mc.inGameScreen.getChatGUI().printChatMessage(component);
    }
    ...
    public void onLivingUpdate() {
        ...
        if (this.onGround && !sneak && !flag2 && this.movementInput.moveForward >= f && !this.isSprinting() && flag3 && !this.isUsingItem() && !this.isPotionActive(Potion.blindness)) {
