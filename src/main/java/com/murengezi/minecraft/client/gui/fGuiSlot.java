...
public abstract class fGuiSlot extends MinecraftUtils {
    ...
    public fGuiSlot(int width, int height, int top, int bottom, int slotHeight) {
        this.width = width;
        this.height = height;
        this.top = top;
        this.bottom = bottom;
    ...
    protected abstract void drawSlot(int entryID, int x, int y, int slotHeight, int mouseX, int mouseY);
    ...
    public int getSlotIndexFromScreenCoords(int x, int y)
    {
        ...
        int k = y - this.top - this.headerPadding + (int)this.amountScrolled - 4;
        int l = k / this.slotHeight;
        return x < this.getScrollBarX() && x >= i && x <= j && l >= 0 && k >= 0 && l < this.getSize() ? l : -1;
    }
