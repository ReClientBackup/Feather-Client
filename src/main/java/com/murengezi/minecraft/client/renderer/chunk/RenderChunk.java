...
import com.murengezi.minecraft.block.BlockCactus;
...
import com.murengezi.minecraft.client.renderer.RegionRenderCache;
...
public class RenderChunk {
    ...
    private final ReentrantLock lockCompileTask = new ReentrantLock(), lockCompiledChunk = new ReentrantLock();
    ...
    private final boolean isMipmaps = Config.isMipmaps(), fixBlockLayer = !Reflector.BetterFoliageClient.exists();
    ...
    private Chunk chunk;
    private final RenderChunk[] renderChunkNeighbours = new RenderChunk[EnumFacing.VALUES.length], renderChunkNeighboursValid = new RenderChunk[EnumFacing.VALUES.length];
    ...
    public void rebuildChunk(float x, float y, float z, ChunkCompileTaskGenerator generator) {
        ...
        if (!this.isChunkRegionEmpty(blockpos)) {
            ...
            for (EnumWorldBlockLayer layer : ENUM_WORLD_BLOCK_LAYERS) {
                ...
                if (compiledchunk.isLayerStarted(layer)) {
                    ...
                    this.postRenderBlocks(layer, x, y, z, worldRenderer, compiledchunk);

                    if (worldRenderer.animatedSprites != null) {
                        compiledchunk.setAnimatedSprites(layer, (BitSet)worldRenderer.animatedSprites.clone());
                    }
                } else {
                    compiledchunk.setAnimatedSprites(layer, null);
                }
            }
    ...
    public ChunkCompileTaskGenerator makeCompileTaskTransparency() {
        ...
        try {
            if (this.compileTask != null && this.compileTask.getStatus() == ChunkCompileTaskGenerator.Status.PENDING) {
                ChunkCompileTaskGenerator generator1 = null;
                return generator1;
            }
            ...
            generator = chunkcompiletaskgenerator;
        } finally {
            this.lockCompileTask.unlock();
        }
    ...
    private void preRenderBlocks(WorldRenderer worldRenderer, BlockPos pos) {
        ...
        if (Config.isRenderRegions()) {
            ...
            j = this.regionX;
            l = this.regionZ;
            worldRenderer.setTranslation(-j, -k, -l);
        } else {
            worldRenderer.setTranslation(-pos.getX(), -pos.getY(), -pos.getZ());
        }
    }

    private void postRenderBlocks(EnumWorldBlockLayer layer, float x, float y, float z, WorldRenderer worldRenderer, CompiledChunk compiledChunk) {
        if (layer == EnumWorldBlockLayer.TRANSLUCENT && !compiledChunk.isLayerEmpty(layer)) {
            worldRenderer.func_181674_a(x, y, z);
    ...
    public BlockPos func_181701_a(EnumFacing facing) {
