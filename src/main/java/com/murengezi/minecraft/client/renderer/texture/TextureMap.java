...
public class TextureMap extends AbstractTexture implements ITickableTextureObject {
	...
	public static final ResourceLocation LOCATION_MISSING_TEXTURE = new ResourceLocation("missingno"), locationBlocksTexture = new ResourceLocation("textures/atlas/blocks.png");
	...
	private int iconGridSize, iconGridCountX, iconGridCountY;
	private double iconGridSizeU, iconGridSizeV;
	private final CounterInt counterIndexInMap;
	public int atlasWidth, atlasHeight;
	private int countAnimationsActive, frameCountAnimations;
		...
		while (true) {
			if (iterator.hasNext()) {
				...
				if (!this.skipFirst) {
					...
					ResourceLocation resourcelocation1 = new ResourceLocation(textureatlassprite3.getIconName()), resourcelocation2 = this.completeResourceLocation(resourcelocation1, 0);
					textureatlassprite3.updateIndexInMap(this.counterIndexInMap);

					if (textureatlassprite3.hasCustomLoader()) {
						if (!textureatlassprite3.load()) {
				...
				for (TextureAtlasSprite textureatlassprite5 : stitcher.getStichSlots()) {
					...
					if (textureatlassprite6 != null) {
						if (textureatlassprite6.getIconWidth() <= 0) {
							...
							ArrayList list = textureatlassprite5.getFramesTextureData();
