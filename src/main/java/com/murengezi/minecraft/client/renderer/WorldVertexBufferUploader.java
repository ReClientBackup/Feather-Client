	...
	public void func_181679_a(WorldRenderer p_181679_1_) {
		if (p_181679_1_.getVertexCount() > 0) {
			...
			for (int j = 0; j < list.size(); ++j) {
				...
				if (flag) {
					Reflector.callVoid(usage, Reflector.ForgeVertexFormatElementEnumUseage_preDraw, vertexformat, j, i, bytebuffer);
				} else {
			...
			for (int k1 = list.size(); j1 < k1; ++j1) {
				VertexFormatElement element = list.get(j1);
				VertexFormatElement.EnumUsage enumUsage = element.getUsage();

				if (flag1) {
					Reflector.callVoid(enumUsage, Reflector.ForgeVertexFormatElementEnumUseage_postDraw, vertexformat, j1, i, bytebuffer);
				} else {
					int i1 = element.getIndex();
