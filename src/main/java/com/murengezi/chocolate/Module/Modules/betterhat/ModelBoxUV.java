package com.murengezi.chocolate.Module.Modules.betterhat;
...
public class ModelBoxUV extends ModelBox
{
    ...
    public static ModelBoxUV addBox(ModelRendererUV renderer, float x, float y, float z, int w, int h, int d, float add) {
        int offsetX = renderer.getTextureOffsetX();
        int offsetY = renderer.getTextureOffsetY();
        ModelBoxUV modelboxuv = new ModelBoxUV(renderer, offsetX, offsetY, x, y, z, w, h, d, add);
    ...
    public ModelBoxUV setSideUV(int side, int textureX, int textureY) {
        QuadData quadData = this.quadDataList[side];
        int i = Math.abs(quadData.uvPos[2] - quadData.uvPos[0]);
        int j = Math.abs(quadData.uvPos[3] - quadData.uvPos[1]);
        quadData.uvPos[0] = textureX;
        quadData.uvPos[1] = textureY;
        quadData.uvPos[2] = textureX + i;
        quadData.uvPos[3] = textureY + j;

        if (side == 3) {
            quadData.uvPos[1] = textureY + j;
            quadData.uvPos[3] = textureY;
        }
    ...
    public ModelBoxUV initQuads() {
        ...
        for (int i = 0; i < this.quadList.length; ++i) {
            QuadData modelboxuv$quaddata = this.quadDataList[i];
    ...
    public void render(Tessellator tessellator, float par2) {
        ...
        for (TexturedQuad texturedQuad : this.quadList) {
            texturedQuad.draw(tessellator.getWorldRenderer(), par2);
        }
    }
