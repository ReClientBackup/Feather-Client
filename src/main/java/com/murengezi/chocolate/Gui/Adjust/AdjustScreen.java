package com.murengezi.chocolate.Gui.Adjust;
...
public class AdjustScreen extends Screen {
	...
	public AdjustScreen() {
		mods = Chocolate.getModuleManager().getModules().stream().filter(module -> module instanceof Adjustable).map(module -> (Adjustable)module).collect(Collectors.toList());
		selected = null;
